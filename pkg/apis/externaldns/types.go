...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("zone-name-filter", "Filter target zones by name (only available for AWS and Azure providers); specify multiple times for multiple zones; Overrides domain-filter for target zones (optional)").Default("").StringsVar(&cfg.ZoneNameFilter)
